    ...
    <div class="mb-6 p-6 bg-white shadow-lg rounded-lg">
        ...
        <form action="{{ route('student.loans.process_repayment', $loan->id) }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="number" name="amount" id="amount"max="{{ $loan->balance ?? $loan->loan_amount }}"min="1" value="{{ old('amount') }}"class="w-full border border-gray-300 rounded px-3 py-2"required>  
            </div>

            <button type="submit" class="bg-blue-500 text-white px-6 py-2 rounded hover:bg-blue-600">Pay Now
            </button>
        </form>
    </div>
    @else
