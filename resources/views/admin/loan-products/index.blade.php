    ...
    <table class="w-full border bg-white">
        ...
        <tbody>
            @foreach($products as $product)
                ...
                <a href="{{ route('admin.loan-products.edit', $product) }}" class="bg-primary-700 hover:bg-primary-500 text-white px-3 py-1 rounded">Edit</a>

                        <form action="{{ route('admin.loan-products.destroy', $product) }}"method="POST"class="inline">
                            @csrf
                            @method('DELETE')
                            <button class="bg-red-600 hover:bg-red-500 text-white px-3 py-1 rounded" onclick="return confirm('Delete this product?')"> Delete
                            </button>
                        </form>
                    </td>
                </tr>
