...
public function disburse($id)
    {
        ...
        try {
            ...
            if ($loan->disbursement && $loan->disbursement->status === 'completed') {
                return back()->with('error', 'Loan already disbursed');
            }

            $phone = preg_replace('/\D/', '', $loan->user->phone);
            if (str_starts_with($phone, '0')) {
                $phone = '254' . substr($phone, 1);
            }

           
            $amount = (int) ($loan->approved_amount ?? $loan->loan_amount);
            if ($amount <= 0) {
                return back()->with('error', 'Invalid disbursement amount');
            }
